...
A Chrome extension for taming a busy browser. Group tabs by domain, sort and
filter them, pause groups to free up memory, and manage everything from a
compact popup or a resizable pop-out window.

Manifest V3, no build step, and **fully self-contained** — no remote scripts,
fonts, or CDNs.

## Features

- **Group by domain** — tabs are bucketed by site so you can see and act on them at a glance.
- **Scope** — work within the current window or across all windows. In the all-windows view each tab shows a small colored pip indicating which window it belongs to.
- **Sort** — most recently used, least recently used, most tabs, or alphabetical (favorites pinned first).
...
- **Selection actions** — with tabs selected, create a group (consolidated into one window or kept per-window), add them to an existing group, move them to a new window, close them, or bookmark them.
...
`localStorage` and does not collect or transmit any data.
